 ...
 *  
 *  NOTE: does NOT use a MyTextField object.  Apparently I wrote this before writing that.
 *  	Should probably use that for text input... oh well
 * 
 * @author Mike Facelle
 *
 */
public class SatelliteLaunch implements Game
{
	...
	// these probably don't need to be global instance variables:
...
// INPUT	
	...
	public void inputMode(int mode)
	{
		...
		while (Keyboard.next() && Keyboard.getEventKeyState()) {
			// re-parse the input value every iteration (after number/backspace input, or enter)
			try {
				// use a substring of [4,length) because ALL input string headers are: "x = "
				// 	so the substring starts at index=4
				// why I did it this way, I do not know...
				// SHOULD have made a mytext to display "x = ", and then made
				//	the actual input string JUST the number... but oh well. I fucked that up
				// doing it this way makes the actual input much simpler (look below the switch-case)
				//	and also allows for scientific E notation (which I COULD have done manually... but that's a pain)
				inputVal = Float.valueOf(inputString.substring(4, inputString.length()));
			}
			catch (NumberFormatException ex) {
				inputVal = 1;	// just set to 1 if invalid number entered (1 to avoid weird 0 stuff)
			}
			...
			// numerical input: append to string and re-parse input value
			...
			/*
			 * Old method: manually parse the input as it happens
			 * 	But this is shittier.  Should have just made a mytext to display "x = ",
			 * 	and then just made the inputString variable ONLY hold the number to be parsed... Oh well.
			 * This method DOES NOT allow for scientific E-notation. which is a mistake
			 * 
			int length = inputString.length();
			if (Keyboard.isKeyDown(Keyboard.KEY_BACK) && length > 0) {
	...
	private void currentSatControlMode() 
	{
		if (Keyboard.isKeyDown(Keyboard.KEY_EQUALS)) {	// actually '+' key (due to Shift)
			if (satellites.size() > 0 && currentSatIndex < satellites.size()) {		// apply forward thrust
			...
			if (satellites.size() > 0 && currentSatIndex < satellites.size()) {		// apply retrograde thrust
				Satellite sat = satellites.get(currentSatIndex);
				// motion all in xy-plane. determine angle to have thrust act on
				float theta = (float)Math.atan(Math.abs(sat.dy() / sat.dx()));
				float fx = -INIT_THRUST*(float)Math.cos(theta);
...
// LOGIC/UPDATE	
	...
	public void updateStrings()
	{
		...
		if (satellites.size() > 0 && currentSatIndex < satellites.size()) {
